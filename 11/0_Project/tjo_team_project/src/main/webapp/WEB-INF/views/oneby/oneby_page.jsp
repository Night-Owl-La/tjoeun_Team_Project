		...
		<div class="content_menu">
			<div class="container">
				<div class="row">
					<!-- 고객 서비스 -->
					<div class="content_item">
						<c:if test="${ empty onebyList }">
                        	작성된 1:1문의가 없습니다.
                        </c:if>
						<c:if test="${ not empty onebyList }">
							<table id="notice_Table" class="table table-bordered table-hover">
								<caption hidden>1:1문의</caption>
								<tbody>
									<tr>
										<th width="10%">No</th>
										<th width="15%">구분</th>
									...
									<c:forEach var="oneby" items="${ onebyList }">
										<tr>
											<td>${ oneby.oneby_idx }</td>
											<td>${ oneby.oneby_category }</td>
											<td>${ oneby.oneby_title }</td>
											<td>${ oneby.oneby_modifydate }</td>
										</tr>
									</c:forEach>
