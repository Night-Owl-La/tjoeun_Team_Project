...
//-----그룹페이지처리 이전 --------------------------------------------------------------------------------------------		
		if (isPrevPage) {
			sb.append(String.format("<a href='' onclick=\"comment_list('%d');return false;\">◀ <a>", startPage-1));
		} else
			sb.append("◀&nbsp");
...
//------페이지 목록 출력 -------------------------------------------------------------------------------------------------
		...
		for (int i = startPage; i <= endPage; i++) {
			...
			if (i == nowPage) { // 현재 있는 페이지
				...
				sb.append(String.format("<a href='' onclick=\"comment_list('%d');return false;\">%d&nbsp<a>", i, i));
			}
		} // end for
		sb.append("&nbsp;|&nbsp");
...
//-----그룹페이지처리 다음 ----------------------------------------------------------------------------------------------
		if (isNextPage) {
			sb.append(String.format("<a href='' onclick=\"comment_list('%d');return false;\">▶<a>", endPage+1));
		} else
			sb.append("&nbsp▶");
//---------------------------------------------------------------------------------------------------------------------	    
